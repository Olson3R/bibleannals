...
export function PeriodPeopleClient({ period, allPeople, allEvents, eventLocationNames, dataMinYear, dataMaxYear }: PeriodPeopleClientProps) {
  ...
  // Get period participant IDs for scoping
  const periodParticipantIds = new Set<string>();
  allEvents.forEach(event => {
    event.participants.forEach(p => periodParticipantIds.add(p));
  });

  // Filter people based on date range and advanced filters (scoped to period participants)
  ...
  const personTypeOptions = Array.from(new Set(
    people.map(p => p.ethnicity).filter((ethnicity): ethnicity is string => Boolean(ethnicity))
  )).sort();
  ...
  const handleEventClick = (event: BiblicalEvent) => {
    const params = new URLSearchParams({ from: 'period-people' });
